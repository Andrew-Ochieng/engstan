...
const Products = ({products}) => {
  ...
  // const filteredData = (cartItem) => {
  //   const result = products.filter((item) => {
  //     return item.product === cartItem
  //   });
  //   setData(result);
  // }
      ...
      <div className="md:mx-32 mx-6 my-8">
        <div className="bg-[#C70B38] rounded-lg py-2 md:px-12 px-2">
          <div className="flex justify-between items-center space-x-2">
            {/* <button onClick={() => setData(products)} className="category-btn">
                All Products
            </button> 
            <button onClick={() => filteredData('Plan')} className="category-btn">
                Plain Products
            </button> 
            <button onClick={() => filteredData('Branded')} className="category-btn">
                Branded Products
            </button> */}
          </div>
        </div>
